...
if (board) {
  ...
  const heading = live ? `Today — ${esc(today.date)}` : "This week";
  const menuItems = menu.map((m) => `<li>${esc(m)}</li>`).join("");
  board.innerHTML = `
    <p class="board-eyebrow">On the kitchen board</p>
    <h3>${heading}</h3>
    <div class="board-cols">
      <div>
        <p class="board-label">Lunch</p>
        <ul class="board-menu">${menuItems}</ul>
      </div>
      <div>
        <p class="board-label">${live ? "Today" : "This week"}</p>
...
if (peopleEl && GD.people) {
  GD.people.forEach((p) => {
    const photo = p.photo
      ? `<img src="${esc(p.photo)}" alt="${esc(p.name)}, ${esc(p.role)}" />`
      : `<div class="photo-ph" role="img" aria-label="Photo of ${esc(p.name)}"><span class="todo-chip">TODO: photo</span></div>`;
    const years = p.years === 1 ? "1 year here" : `${esc(p.years)} years here`;
    peopleEl.append(
      el(
        "li",
        "person",
        `${photo}<p class="person-name">${esc(p.name)}</p><p class="person-role">${esc(p.role)}</p><span class="person-years">${years}</span>`
      )
    );
  });
}
...
if (voicesEl && GD.voices) {
  GD.voices.forEach((v) => {
    ...
    const video = v.photo
      ? `<img src="${esc(v.photo)}" alt="Video message from ${esc(v.name)}" />`
      : `<div class="voice-video"><div class="photo-ph" role="img" aria-label="Video message from ${esc(v.name)}"><span class="todo-chip">TODO: video</span></div></div>`;
    const reply = v.reply ? `<div class="voice-reply">${esc(v.reply)}</div>` : "";
    voicesEl.append(
      el(
        "li",
        "voice",
        `${video}<div class="voice-stars" aria-label="${v.stars} out of 5">${stars}</div>` +
...
if (form) {
  form.addEventListener("submit", (e) => {
    ...
    if (!name) return fail(msg, "Please add your name so Carmen knows who to ask for.");
    if (digits.length < 7) return fail(msg, "Please add a phone number Carmen can call you on.");
    const time = (form.querySelector('input[name="visit-time"]:checked') || {}).value || "";
    form.hidden = true;
    const done = $("#visit-done");
    done.hidden = false;
    $("#visit-done-detail").textContent = time
      ? `We've noted ${time}. Carmen will call ${name} to confirm — no pressure either way.`
      : `Carmen will call ${name} to find a time that suits you.`;
